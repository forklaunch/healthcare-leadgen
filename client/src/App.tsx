...
type Tab = 'how' | 'submit' | 'admin';

const KEY_STORAGE = 'uchicago-ideas-access-key';
...
function randomCipher(n: number): string {
  let s = '';
  for (let i = 0; i < n; i++) {
...
const SHOWCASE_FIELDS = [
  { label: 'name', value: 'Dr. Jane Smith' },
  { label: 'email', value: 'jsmith@uchicagomedicine.org' },
...
function EncryptionShowcase() {
  ...
  return (
    <div className="card demo-card">
      ...
      <p>
        Every name, email, and idea is sealed with AES-256-GCM{' '}
        <em>before</em> it is written to disk. This is what a record actually
        looks like inside our database — open it the way an access key does.
      </p>
      <div className="vault">
...
function RequestAccessCard({ onDone }: { onDone: (msg: string) => void }) {
  ...
  return (
    <div className="card">
      ...
      <p>
        Enter your details and we will email you a secure access link. The link
        is your key to the portal — no password required.
      </p>
      <label>Full name</label>
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Dr. Jane Smith" />
      <label>UChicago Medicine email</label>
      <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="jsmith@uchicagomedicine.org" />
  ...
  return (
    <div className="card">
      ...
      <p>
        ...
        accepting below; UChicago Medicine counter-signs automatically, and
        both execution timestamps are retained.
      </p>
...
function SubmitTab({
    ...
    return (
      <>
        <section className="hero">
          <p className="eyebrow">UChicago Medicine · Physician Innovation Program</p>
          <h2>
            Your clinical insight, <em>built</em> into real software.
          </h2>
          <p className="lede">
            Pitch the fix your unit needs. Ideas are sharpened by BAA-covered
            AI, protected by a mutual NDA, and sealed with hospital-grade
            encryption — and the top seven get built by ForkLaunch engineers.
          </p>
        ...
        <RequestAccessCard onDone={setFlash} />
        <EncryptionShowcase />
      </>
    );
  }
  ...
  if (!me.ndaAccepted) {
    return <NdaGate accessKey={accessKey} me={me} onAccepted={refreshMe} />;
  }
  ...
  const runEvaluation = async (focusKey: EvaluationFocus | null) => {
    ...
    setError(null);
    try {
      const result = await evaluateIdea(accessKey, {
        title,
        body,
  ...
  const focusAndImprove = async (dimKey: EvaluationFocus) => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        if (!dim) break;
        const refined = await refineIdea(accessKey, {
          ...draft,
          focus: dimKey,
        ...
        const newEval = await evaluateIdea(accessKey, {
          ...draft,
          focus: dimKey
        });
    ...
    <>
      <div className="card">
        ...
        <p className="notice ok" style={{ marginTop: '1rem' }}>
          ...
          your draft scored by our AI evaluator. It runs on an Azure AI Foundry
          endpoint covered by a Business Associate Agreement (BAA), so it is
          held to the same HIPAA safeguards as the rest of this portal: your
          draft is encrypted in transit, is never used to train models, and is
          not retained by the AI provider. Nothing is stored in the portal
        ...
        <div className="btn-row">
          ...
          <button
            ...
            onClick={async () => {
              ...
              setError(null);
              try {
                await submitIdea(accessKey, { title, body });
                setTitle('');
                setBody('');
                setEvaluation(null);
                setPrevScores(null);
...
function StorageInspectorCard({ accessKey }: { accessKey: string }) {
  const [data, setData] = useState<StorageInspector | null>(null);
  const [decrypted, setDecrypted] = useState(false);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    getStorageInspector(accessKey, false).then(setData);
  }, [accessKey]);
    ...
    <div className="card demo-card">
      ...
      <button
        ...
        onClick={async () => {
          if (decrypted) {
            setBusy(true);
            const d = await getStorageInspector(accessKey, false);
            setData(d);
            setDecrypted(false);
            setBusy(false);
          } else {
            setBusy(true);
            const d = await getStorageInspector(accessKey, true);
            setData(d);
            setDecrypted(true);
            setBusy(false);
          }
    ...
    <>
      {me && accessKey && (
        <>
          ...
          <StorageInspectorCard accessKey={accessKey} />
        </>
      )}
      <div className="card">
        ...
        <ol className="steps">
          ...
          <li>
            ...
            sealed form below, and unseal it yourself.
          </li>
          <li>
      ...
      <div className="card">
        ...
        <p>
          ...
          <strong>Columbia</strong>, among others. Your clinical insight, their
          engineering — on the same compliance-enforced platform this portal
          runs on.
        </p>
      </div>
...
function AdminTab() {
  ...
  useEffect(() => {
    if (token) {
      adminListIdeas(token).then(setIdeas).catch((e) => setError(e.message));
    }
  }, [token]);
      ...
      <div className="card">
        ...
        <button
          ...
          onClick={async () => {
            ...
            setError(null);
            try {
              setToken(await adminLogin(email, password));
            } catch (e) {
            ...
            {ideas.map((idea) => (
              <tr key={idea.id}>
                <td>
                  ...
                  <div className="meta">{idea.body}</div>
                </td>
                <td>
                  {idea.doctorName}
                  <div className="meta">
...
export default function App() {
  ...
  const refreshMe = useCallback(() => {
    if (accessKey) getMe(accessKey).then(setMe);
  }, [accessKey]);
  ...
  return (
    <>
      <header className="masthead">
        <div className="shell">
          <p className="eyebrow">The University of Chicago Medicine</p>
          <h1>Ideas Portal</h1>
          <p>
            A confidential waitlist for physician innovation ideas · HIPAA
            simulation — a demonstration of PHI-grade data handling
          </p>
        </div>
      </header>
      <nav className="tabs">
      ...
      <main>
        {tab === 'how' && <HowItWorksTab me={me} accessKey={accessKey} />}
        {tab === 'submit' && <SubmitTab accessKey={accessKey} me={me} refreshMe={refreshMe} />}
        {tab === 'admin' && <AdminTab />}
      </main>
    </>
  );
